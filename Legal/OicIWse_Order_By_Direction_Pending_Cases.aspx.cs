...
public partial class Legal_OicIWse_Order_By_Direction_Pending_Cases : System.Web.UI.Page
{
    DataSet dsCase = null;
    ...
    protected void BindGrid(string CaseType,int OicId)
    {
        ...
        {
            int Oicid = 1;
            dsCase = obj.ByDataSet("select distinct UniqueNo,CaseType," +
                ...
                ",Remarks,HearingDate,CaseNo,OrderComplianceDate,IsComplaince,CaseSubSubjectId,IsOrderByDirection,RespondentOfficeId " +
                "from tbl_OrderByDirectionPendingCase a where CaseType='" + Convert.ToString(CaseType) + "' and OicId=" + Oicid);
            if (dsCase.Tables.Count > 0 && dsCase.Tables[0].Rows.Count > 0)
            {
                ViewState["dt"] = null;
    ...
    protected void grdCaseTypeDetail_RowUpdating(object sender, GridViewUpdateEventArgs e)
    {
        ...
        strQuery += "',Respondent='" + convertQuotes(txtRespondent.Text.Trim()) + "' where UniqueNo='" + Convert.ToString(hdnUId.Value) + "' and Oicid =" + OicId;
    ...
    protected void btnSearch_Click(object sender, EventArgs e)
    {
        try
        {
          ...
          ",Remarks,HearingDate,CaseNo,OrderComplianceDate,IsComplaince,CaseSubSubjectId,IsOrderByDirection,RespondentOfficeId  from tbl_OrderByDirectionPendingCase a " +
           "where CaseType='" + Convert.ToString(Request.QueryString["CaseType"]) + "' and FilingNo like '%" + Convert.ToString(txtSearch.Text.Trim()) + "%' and OicId=" + OicId);
            if (dsCase.Tables[0].Rows.Count > 0)
            {
